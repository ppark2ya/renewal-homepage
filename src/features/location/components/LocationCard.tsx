        ...
        <div className="flex gap-3">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-1 mb-2">
              ...
              <Image src="/images/flags/cn.svg" alt="CNY" width={16} height={12} className="object-cover rounded-sm" />
              <Image src="/images/flags/us.svg" alt="USD" width={16} height={12} className="object-cover rounded-sm" />
              <Image src="/images/flags/jp.svg" alt="JPY" width={16} height={12} className="object-cover rounded-sm" />
            </div>
            ...
            {/* Operating Hours */}
            <p className="text-[12px] leading-[18px] text-[#717895] mb-2">
            ...
            <div className="flex flex-wrap gap-1 text-[12px] text-[#717895]">
              {location.services.map((service, idx) => (
                <span key={service}>
                  {service === 'foreignToKrw' && 'KRW → Foreign'}
                  {service === 'krwToForeign' && 'Foreign → KRW'}
                  {service === 'taxRefund' && 'TAX REFUND'}
                  {service === 'ezlCardTopup' && 'Prepaid card'}
                  {idx < location.services.length - 1 && ' / '}
                </span>
              ))}
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 mb-5">
        ...
        <p className="text-[14px] leading-[20px] text-[#717895]">
          {location.operatingHours}
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        {location.services.map((service) => (
          <ServiceBadge key={service} type={service} />
        ))}
      </div>
    </div>
  );
